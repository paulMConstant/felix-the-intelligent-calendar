...
fn test_filter_conflicts_real_life() {
    ...
    assert_eq!(
        ...
        vec![InsertionCostsMinutes {
            ...
            // Base cost 10000 divided by number of inserted activities + 1
            cost: 10000 / 4,
        }]
    );
}
...
fn test_insertion_costs_simplest() {
    ...
    assert_eq!(insertion_costs0[0], 10000 + 10000); // 0 Blocks 0, 5
                                                    // -> 2 blocked * 1 incompatible activities / 2 remaining spots
    assert_eq!(insertion_costs0[1], 10000 + 30000); // 5 Blocks 0, 5, 10
                                                    // -> 3 blocked * 1 incompatible activities / 1 remaining spots
    assert_eq!(insertion_costs0[2], 10000 + 30000); // 10 Blocks 0, 5, 10
    assert_eq!(insertion_costs0[3], 10000 + 30000); // 15 Blocks 5, 10 20
    assert_eq!(insertion_costs0[4], 10000 + 10000); // 20 Blocks 10, 20
    assert_eq!(insertion_costs0[5], 10000 + 0); // 35 Blocks nothing
    assert_eq!(insertion_costs0[6], 10000 + 0); // 45 Blocks nothing
    assert_eq!(insertion_costs0[7], 10000 + 0); // 50 Blocks nothing
    ...
    assert_eq!(insertion_costs1[0], 10000 + 6000); // 0 Blocks 0, 5, 10
                                                   // -> 3 blocked * 1 incompatible activities / 5 remaining spots
    assert_eq!(insertion_costs1[1], 10000 + 10000); // 5 Blocks 0, 5, 10, 15
                                                    // -> 4 blocked * 1 incompatible activities / 4 remaining spots
    assert_eq!(insertion_costs1[2], 10000 + 10000); // 10 Blocks 5, 10, 15, 20
    assert_eq!(insertion_costs1[3], 10000 + 3333); // 15 Blocks 10, 20
