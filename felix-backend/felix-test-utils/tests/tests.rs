...
use felix_backend::{TimeInterval, Time, MIN_TIME_DISCRETIZATION};
...
fn add_entities() {
    ...
    test_ok!(
        data,
        DataBuilder::new().with_entities(vec![entity1, entity2]),
        {
            let entities = data.entities_sorted();
            ...
            assert_eq!(
                entities[0].name(),
                entity1,
                "Entity was not added with the right name or entities are not sorted"
            );
            assert_eq!(
                entities[1].name(),
                entity2,
    ...
    test_ok!(
            ...
            .with_custom_work_interval_for(entity, interval),
        {
            ...
            let custom_work_hours = data
                .custom_work_hours_of(entity.name())
    ...
    test_ok!(
            ...
            .with_custom_work_intervals_for(entity, vec![interval1, interval2]),
        {
            ...
            assert_eq!(
                ...
                2,
                "Custom work hours were not added"
            );
            assert_eq!(
                custom_work_hours[0], interval1,
                "The wrong interval was added"
            );
            assert_eq!(
                custom_work_hours[1], interval2,
                "The wrong interval was added"
            );
        }
    );
...
fn add_group() {
    ...
    test_ok!(
            ...
            }),
        {
            ...
            assert_eq!(
                groups[0].entities_sorted(),
                vec![entity1, entity2],
                "Entities were not added to the group"
            );
        }
    );
}
...
fn add_default_group() {
    ...
    test_ok!(
        ...
        DataBuilder::new().with_group(Group::default(group_name)),
        {
            ...
            assert!(
                group.entities_sorted().is_empty(),
                "Default group members is wrong"
            );
        }
    );
}
...
fn add_groups() {
    ...
    test_ok!(
        data,
        DataBuilder::new().with_groups(vec![Group::default(group1), Group::default(group2)]),
        {
            let groups = data.groups_sorted();
            ...
            assert!(
                groups[0].entities_sorted().is_empty(),
                "Default group members is wrong"
            );
            assert!(
                groups[1].entities_sorted().is_empty(),
                "Default group members is wrong"
            );
        }
    );
...
fn add_work_interval_with_duration() {
    test_ok!(
        data,
        DataBuilder::new().with_work_interval_of_duration(4),
        {
    ...
    test_ok!(
        ...
        DataBuilder::new().with_activity(Activity::default()),
        {
            ...
            assert_eq!(
                activity.name(),
                "Activity",
                "Default activity name is wrong"
            );
            assert_eq!(
                activity.duration(),
                MIN_TIME_DISCRETIZATION,
                "Default activity duration is wrong"
            );
            assert!(
                activity.entities_sorted().is_empty(),
                "Default activity members is wrong"
            );
            assert!(
