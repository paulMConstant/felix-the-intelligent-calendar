use plan_backend::errors::name_taken::{GroupOrEntity, NameTaken};
...
fn en_display_name_taken_by_entity() {
    ...
    assert_eq!(
        error.to_string(),
        "The name 'Entity Name' is already taken by an entity."
    );
}
...
fn en_display_name_taken_by_group() {
    ...
    assert_eq!(
        error.to_string(),
        "The name 'Group Name' is already taken by a group."
    );
}
