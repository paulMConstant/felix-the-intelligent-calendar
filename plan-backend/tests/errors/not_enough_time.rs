...
fn en_display_activity_added() {
    ...
    assert_eq!(error.to_string(), "Entity Name will not have enough time if they are added to 'Activity'.");
}

#[test]
fn en_display_activity_duration_increased() {
    let error = NotEnoughTime::activity_duration_too_long_for("Entity Name", "Activity");
    assert_eq!(error.to_string(), "Entity Name will not have enough time if the duration of 'Activity' is increased.");
}
