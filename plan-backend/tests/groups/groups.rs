...
fn add_group_check_sorting() {
    ...
    test_ok!(data, DataBuilder::new().with_groups(vec![Group::default(name1), Group::default(name2)]),
             {
    let groups = data.groups_sorted();
...
fn simple_rename_group() {
    ...
    test_ok!(data, DataBuilder::new().with_group(Group::default(group_name)), {
    data .set_group_name(group_name, new_name) .expect("Could not rename group");
...
fn rename_group_name_taken_by_group() {
    ...
    test_err!(data, DataBuilder::new().with_groups(vec![Group::default(group_name),
    Group::default(other_name)]),
