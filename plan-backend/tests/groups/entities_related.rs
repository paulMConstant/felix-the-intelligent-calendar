...
use test_utils::data_builder::{Group, Activity, DataBuilder};
...
fn add_entity_to_group_already_in_group() {
    let group_name = "Group";
    let entity_name = "Entity";
    test_err!(data, DataBuilder::new().with_entity(entity_name)
...
fn add_nonexistent_entity_to_group() {
    let group_name = "Group";
    test_err!(data, DataBuilder::new().with_group(Group::default(group_name)), 
...
fn add_entity_to_nonexistent_group() {
    let entity = "Entity";
    test_err!(data, DataBuilder::new().with_entity(entity), 
...
fn add_entity_to_group_empty_group_name() {
    let group = "Group";
    test_err!(data, DataBuilder::new().with_group(Group::default(group)), 
...
fn add_entity_to_group_check_sorting() {
    ...
    test_ok!(data, DataBuilder::new().with_entities(vec![entity1, entity2])
             .with_group(Group{name:group, entities: vec![entity2, entity1]}), {
    let entities = data
        .group(group)
...
fn add_entity_to_group_not_enough_time_left() {
    ...
    test_err!(data, DataBuilder::new().with_entity(entity)
              .with_group(Group::default(group))
                  .with_activity(Activity {groups: vec![group], duration: Time::new(1, 0),
                  ..Default::default()}),
            data.add_entity_to_group(group, entity),
...
fn simple_remove_entity() {
    let (entity1, entity2, group) = ("Entity1", "Entity2", "Group");
    test_ok!(data, DataBuilder::new().with_entities(vec![entity1, entity2])
             .with_group(Group{name:group, entities: vec![entity1, entity2]}), {
